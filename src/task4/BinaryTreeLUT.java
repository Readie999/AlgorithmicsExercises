...
public class BinaryTreeLUT {
  ...
  protected BSTreeNode lrMerge(BSTreeNode node) {
    BSTreeNode mergedTrees, newnode = null;
    ...
    if (node.left == null) {
      ...
      mergedTrees = node.left;
    	...
    	newnode= node.left;
    	//this gives the right most node in the left subtree.x
     ...
     //mergedTrees = node.left;
    	...
    	try {
    		//remove the right most node in left subtree
			removeFromTree(newnode.kvPair.key, root);
			//set the left and right links to what they were for node to be deleted
